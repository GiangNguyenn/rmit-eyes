...
export default function UserDescriptionDetail(props) {
  ...
  useEffect(() => {
    if (queueUsers && sid && !queueUsers.includes(sid) && temp &&  temp < 39 && status==='approved') {
      const result = axios.post('/users/check-in/', {
        sid: sid, temperature: temp
      }).then(res => {
  ...
  return (
    <div style={{
      ...
      {checkedList.includes(sid) ? <div><img width={25} height={25} src={IMAGE_URLS[status]}/> automatically checked user in.
        This student can access campus now!</div> : <Spin size="large"/>}
    </div>
  );
}
